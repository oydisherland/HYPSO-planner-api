...
from input_data_HYPSO.input_data_utility_functions import getTLEfilePath
...
def createSatelliteObject(HYPSOnr: int) -> skf.EarthSatellite:
    ...
    hypsoTlePath = getTLEfilePath(HYPSOnr)
...
def findDaylightPeriods(targetLat: float, targetLong: float, startTime: datetime.datetime, endTime: datetime.datetime) -> list:
    """ Find the periods where the target is illuminated by the sun within the timeinterval of startTime and endTime
    Output:
    - List of tuples (timeOfSunrise, timeOfSunset)
    """
    ...
    if events.size == 0:
        return daylightPeriods

    if events[0] == 0:
        # first event is sunset, add a timestamp at start of OH
        daylightPeriods.append((t0.utc_datetime(), times[0].utc_datetime()))
    ...
    for time_curr, event_curr, time_next, event_next in zip(times[:-1], events[:-1], times[1:], events[1:]):
        ...
        timeOfSunrise = time_curr.utc_datetime()
        timeOfSunset = time_next.utc_datetime()
        daylightPeriods.append((timeOfSunrise, timeOfSunset))

    if events[-1] == 1:
        # If last event is rise, add a timestamp at end of OH
        daylightPeriods.append((times[-1].utc_datetime(), t1.utc_datetime()))
